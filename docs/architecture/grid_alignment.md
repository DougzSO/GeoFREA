...
1. **Três variantes truncadas da mesma fórmula geodésica** (§b, achado novo): `data_auditor.py`, e duas funções em `grid_aligner.py`, implementam versões com números de termos diferentes da correção Bowring por latitude. **Pergunta**: no GeoFREA, vale centralizar numa única função (`src/utils/geodesy.py`?) usada por todas as fases que precisam de área/distância geodésica, eliminando a divergência de precisão entre elas? Isso incluiria também a fórmula usada em `build_pixel_area_array()` (Fases 4/5, não lida em detalhe nesta auditoria — candidato a comparação futura).
   ...
   **RESOLVIDO 2026-09-09** (Passo 4 item 1, ver `docs/DECISIONS.md` mesma data): sim, centralizado em `src/geofrea/core/geodesy.py::wgs84_km_per_degree()` (precisão completa), usado pelas 3 chamadas do GeoFREA (`row_area_km2()`, `calculate_wgs84_isotropic_distance()`, resolução adaptativa). Erro medido das duas variantes truncadas contra a completa, nas latitudes reais de PRT/BRA: ~1e-6% a ~1e-3% — desprezível; centralização decidida por manutenibilidade, não por correção de erro. `build_pixel_area_array()` (Fases 4/5) segue fora de escopo, não construído ainda no GeoFREA.
2. **Limite de distância de rios (50 km) hardcoded, diferente do de grid/roads (100 km)**: não há explicação no código sobre por que rios têm um raio de busca menor. **Pergunta**: isso é uma decisão metodológica deliberada (rios "contam" apenas localmente) ou um valor arbitrário que deveria ser unificado/parametrizado?
   ...
   **RESOLVIDO 2026-09-09** (Passo 4 item 2, ver `docs/DECISIONS.md` mesma data): valor arbitrário, sem fundamento — unificado em 100km (`core/constants.py::LINEAR_FEATURE_MAX_DIST_KM`). Confirmado, antes de decidir, que a divergência não tinha efeito no resultado final: `criteria_builder.py` downstream (Fase 2b/3, ainda não construída no GeoFREA) usa distâncias de decaimento muito menores (5-30km) em todos os casos, bem abaixo de ambos os caps antigos.
3. **Resolução adaptativa**: `target_pixels=50000` (tamanho-alvo da grade, controlando o trade-off entre performance e granularidade espacial) não tem justificativa documentada em `parameters.json`/`DECISIONS.md`. **Pergunta**: esse valor foi calibrado empiricamente (ex. tempo de execução aceitável) ou é uma escolha arbitrária que poderia mudar no GeoFREA?
   ...
   **RESOLVIDO 2026-09-09** (Passo 4 item 3, ver `docs/DECISIONS.md` mesma data — achado mais sério dos quatro): `target_pixels` não tem calibração documentada em lugar nenhum, e não controla a resolução de países grandes na prática (BRA sempre bate no teto `max_deg`, independente do valor). Mais grave: o legado real (`configs/settings.yaml`) usava resolução FIXA `0.01°`, não `"adaptive"` — o modo adaptive existia no código mas nunca foi o valor configurado na execução que gerou o baseline congelado (confirmado inspecionando os rasters do baseline: PRT/BRA, ambos 0.01°). GeoFREA agora porta 0.01° fixo como padrão (`config/settings.yaml`'s `geospatial.resolutions.suitability`), com `"adaptive"` disponível como opt-in explícito, não mais o único comportamento.
4. **AHP para combinação de alturas de vento**: o `WIND_AHP_MATRIX` (matriz de comparação pareada Saaty) não foi lido em detalhe nesta auditoria (vive em `src/core/constants.py`). **Pergunta futura**: qual a justificativa dos julgamentos pareados específicos usados nessa matriz (por que 50m/100m/200m recebem esses pesos relativos)?
   ...
   **RESOLVIDO (parcialmente) 2026-09-09** (Passo 4 item 4, ver `docs/DECISIONS.md` mesma data): matriz lida em detalhe; RC≈0.034 (limiar 0.10 confirmado padrão Saaty 1980, não arbitrário), pesos induzidos ≈63/26/11% (200m/100m/50m). Os julgamentos pareados específicos continuam sem fonte documentada em todo o legado — mantidos como `STRUCTURAL_PRESERVE`, mas registrados como pergunta aberta para quando `suitability_criteria` (Fase 3) for desenhada de verdade, não uma pendência fechada.
