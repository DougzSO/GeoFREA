...
- **Unfiltered land_cover registry for BRA and PRT (found 2026-09-22, audit FD3b1/FD3b2).** `resolve_land_cover_tiles()` (`local_layers.py:398-479`) does receive `country_gdf` at its production call site (`phase.py:182-184`) and does compute a real bbox-vs-GADM-polygon intersection filter, but the `data_acquisition` runs currently on disk for both countries predate its effective exclusion: `outputs/BRA/manifest.json`'s `land_cover` layer references 154 of 155 tiles (only `ESA_WorldCover_10m_2020_v100_S36W057_Map.tif` excluded), and `outputs/PRT/manifest.json`'s references all 26 of 26 (0 excluded). A direct geometry check against the real GADM polygons (`docs/_audit/2026-09_geofrea_data_inventory.md` Action 6) finds 42 BRA tiles and 15 PRT tiles with zero overlap — none of which were excluded in the runs on disk. `config/countries.yaml`'s current 44-entry `excluded_land_cover_tiles` list for BRA postdates the BRA manifest's last write (file mtime after manifest mtime). Every downstream artifact for both countries that consumed this land_cover registry (`grid_alignment`, `suitability_criteria`, regardless of their own manifest status) inherits this staleness. Re-running `data_acquisition` for BRA and PRT against the current config is required before either country's `grid_alignment`/`suitability_criteria` can be trusted.
